...
const Checkout: React.FC = () => {
  ...
  const currentStep: Step = useCallback(
    () => checkoutSteps[currentStepId] as Step,
    [currentStepId],
  )();
  ...
  const nextStep = () => {
    if (currentStepId < checkoutSteps.length + 1) {
      setCurrentStepId(
        checkoutSteps.findIndex((step) => step.position === currentStepId + 1)
          + 1,
      );
    }
  };
  ...
  const handleClickBackButton = () => {
    const previousStep = checkoutSteps.find(
      (step) => step.position === currentStepId,
    );
  ...
  return (
    <>
...

      {
        ...
        || selectedCartItem.product === undefined ? (
          <></>
          ) : (
            <>
              <main>
